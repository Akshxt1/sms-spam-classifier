...
# High-signal spam words — never remove these as stopwords
SPAM_KEYWORDS = {
...
def extract_signals(text: str) -> dict:
    """Return raw signal flags from the ORIGINAL text before normalization."""
...
def clean_text(text: str, stem: bool = True) -> str:
    """Normalize, tokenize, remove stopwords, optionally stem."""
    if not isinstance(text, str) or not text.strip():
        return ""

    text = text.lower().strip()

    # Replace signals with semantic tokens before stripping special chars
    text = re.sub(r"https?://\S+|www\.\S+", " URLTOKEN ", text)
    text = re.sub(r"\S+@\S+\.\S+", " EMAILTOKEN ", text)
    text = re.sub(r"\b(\+?\d[\d\s\-]{8,14}\d)\b", " PHONETOKEN ", text)
    text = re.sub(r"[$₹€£¥]\s?\d+[\d,]*|\b\d+[\d,]*\s*(?:rs|usd|gbp|inr)\b", " MONEYTOKEN ", text, flags=re.I)
    text = re.sub(r"\d+", " NUMTOKEN ", text)

    # Keep only alphanumeric and our injected tokens
    text = re.sub(r"[^a-zA-Z\s]", " ", text)

    tokens = text.split()
    cleaned = []
    for tok in tokens:
        # Always keep semantic tokens
